...
def search(driver,city,isdebug,web_order):
    try:#浏览器打开网页
        ...
        return 
    s=(driver.find_element_by_xpath("/html[@class='index_page']/body/div[@class='pagination2']/a[10]").text)#.encode('utf-8')  #获取公司总数目
    print s
    try:
        s=s.replace(u'下','').replace(u'一','').replace(u'页','').replace('>','').replace('.','').replace(' ','')
    ...
    url_num=page_num*20
    # url_num = (driver.find_element_by_xpath("/html[@class='index_page']/body/div[@class='pagination2']").text.replace(u'共','').replace(u'纪录',''))
    # print url_num
