...
- 如果字母的出现次数大于 *0* 且小于或等于 *n/2*，且 ![\textit{oddIndex} ](./p__textit{oddIndex}_.png)  没有超出数组下标范围，则将字母放置在 ![\textit{oddIndex} ](./p__textit{oddIndex}_.png) ，然后将 ![\textit{oddIndex} ](./p__textit{oddIndex}_.png)  的值加 *2*。
