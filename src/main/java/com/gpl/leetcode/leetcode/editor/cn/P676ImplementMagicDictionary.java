...
class MagicDictionary {
    ...
    public boolean search(String searchWord) {
        ...
        return search(searchWord, 0, 1, tree.root);
    }

    private boolean search(String word, int i, int num, TrieNode root) {
        if (num < 0) {
            return false;
        }
        if (i == word.length()) {
        ...
        for (int j = 0; j < 26; j++) {
            ...
            if (idx == j) {
                if (search(word, i + 1, num, root.nexts[idx])) {
                    return true;
                }
            } else if (search(word, i + 1, num - 1, root.nexts[j])) {
                return true;
            }
        }
        return false;
